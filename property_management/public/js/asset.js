...
frappe.ui.form.on("Asset", {
    location: function(frm) {
        if (!frm.doc.location) {
            frappe.msgprint(__('Please provide a location first.'));
            return;
        }

        frappe.call({
            method: "property_management.api.location.get_lon_lat",
            args: {
                name: frm.doc.name,
                address: frm.doc.location,
            },
            callback: function(r) {
                if (r.message) {
                    frm.set_value("latitude", r.message[0]);
                    frm.set_value("longitude", r.message[1]);
                    frm.refresh_field("latitude");
                    frm.refresh_field("longitude");
                } else {
                    frappe.msgprint(__('Unable to retrieve latitude and longitude for the provided address.'));
                }
            },
        });
    },
        ...
        if (frm.doc.latitude && frm.doc.longitude) {
            frm.add_custom_button(__('Open in Google Maps'), function() {
                ...
                if (lat && lng) {
                    ...
                    frappe.msgprint(__('Latitude and Longitude are not available. Please ensure they are filled.'));
                }
            });
        }
    },
